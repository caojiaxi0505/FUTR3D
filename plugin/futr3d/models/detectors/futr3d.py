...
class FUTR3D(MVXTwoStageDetector):
    ...
    def plt_heatmap(self, x, coors=None):
        ...
        if coors is not None:
            ...
            # 计算余弦相似度
            similarity_map = torch.matmul(all_normalized, target_normalized.unsqueeze(-1)).squeeze(-1)  # [B, H, W]
            # 绘制相似度热力图
            similarity_map_np = similarity_map[0].cpu().numpy()  # 可视化batch为1的heatmap
    ...
    def forward_pts_train(self,
        ...
        """
        # Pass the backbone features to the head during training as well
        outputs_classes, outputs_coords, aux_outs = self.pts_bbox_head(pts_feats, img_feats, radar_feats, img_metas, backbone_feat=None)
    ...
    def simple_test_pts(self, pts_feats, img_feats, radar_feats, img_metas, rescale=False):
        ...
        # Pass the backbone features to the head
        outputs_classes, outputs_coords, aux_outs = self.pts_bbox_head(pts_feats, img_feats, radar_feats, img_metas, backbone_feat=None)
    ...
    def simple_test(self, img_metas, points=None, img=None, radar=None, rescale=False, gt_bboxes_3d=None, gt_labels_3d=None):
