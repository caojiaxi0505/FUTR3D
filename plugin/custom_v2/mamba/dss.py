...
class DSSMamba(nn.Module):
    def __init__(
        ...
        xy=False,           # 是否morton使用横纵交错
        kernel_size=4,           # conv1d的kernel_size，一般情况下不作修改
        # <<< morton重排以及conv1d路径 <<<
        # >>> 一般情况不应修改的参数 >>>
        dt_rank="auto",
        dt_min=0.001,
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList([
            nn.ModuleDict({
                ...
                "dropout": DropPath(drop_rate) if drop_rate > 0 else nn.Identity(), # drop_rate为0不添加dropout
                "norm": nn.LayerNorm(d_model) if i < num_layers - 1 else nn.Identity(), # 最后一层不添加norm
            })
