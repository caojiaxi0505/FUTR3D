...
class MSSMamba(nn.Module):
    def __init__(
        ...
        self.norm_HF = nn.LayerNorm(self.d_model, eps=1e-6, **factory_kwargs)
        self.norm_HB = nn.LayerNorm(self.d_model, eps=1e-6, **factory_kwargs)
        self.norm_VF = nn.LayerNorm(self.d_model, eps=1e-6, **factory_kwargs)
        self.norm_VB = nn.LayerNorm(self.d_model, eps=1e-6, **factory_kwargs)
    ...
    def forward(self, pts_feats: list, forepred: list) -> list:
        ...
        out = self.out_act(torch.cat([self.norm_HF(out_HF), self.norm_HB(out_HB), self.norm_VF(remap_vertical_to_horizontal(out_VF, morton_V_indices, morton_H_indices)), self.norm_VB(remap_vertical_to_horizontal(out_VB, morton_V_indices, morton_H_indices))], dim=-1))
    ...
    def get_indices(self, forepred: list) -> tuple:
        ...
        morton_indices_list_2 = []
        batch_size = forepred[0].shape[0]
        for pred_mask in forepred:
            morton_indices_batch_1 = []
            morton_indices_batch_2 = []
            for b in range(batch_size):
                current_mask = pred_mask[b, 0]
                total_elements = current_mask.numel()
                k = int(total_elements * 0.02)
                if k > 0:
                    threshold = torch.topk(current_mask.flatten(), k, largest=True).values[-1]
                    mask = current_mask >= threshold
                else:
                    mask = current_mask == current_mask.max()
                idx_1, idx_2 = self.morton_code_extraction(mask)
                morton_indices_batch_1.append(idx_1)
                morton_indices_batch_2.append(idx_2)
            morton_indices_list_1.append(morton_indices_batch_1)
...
def generate_foregt(multi_scale_batch_bev_feats: list, gt_bboxes: list, bev_scales: tuple) -> list:
    ...
    for bev_feat in multi_scale_batch_bev_feats:
        ...
        gt_foreground.append(foreground_mask.permute(0,1,3,2))
