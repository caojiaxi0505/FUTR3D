...
class FUTR3DHead(DETRHead):
    def __init__(self,
                 *args,
                 ...
                 use_hybrid=False,
                 hybrid=None,
                 dss_batch_first=False, # 尽可能不改
                 dss_drop_prob=0.1,
                 dss_mamba_prenorm=False,   # 尽可能不改
                 dss_mamba_cfg=dict(d_model=256, d_state=128, expand=1),
                 dss_mamba_version="dss_mamba",
                 dss_num_layers=3,
                 dss_rope=False,
            ...
            if use_hybrid:
                ...
                for lid in hybrid:
                    self.transformer.decoder.layers[lid].attentions[0] = DSS(
                        ...
                        use_rope=dss_rope,
                        morton_rearrange=dss_morton_rearrange,
                        conv_path=dss_conv_path,
                        xy=dss_xy,
                        d_model=256,
                        deepseek_format=dss_deepseek_format)
            else:
        ...
        if use_mss:
            ...
            self.ForePredNet = nn.ModuleList([
                ForePredNet(mss_fore_pred_net_in_channels, mss_fore_pred_net_intermediate_channels, mss_fore_pred_net_out_channels) for _ in range(mss_num_scales)])
    ...
    def forward(self, mlvl_pts_feats, mlvl_img_feats, radar_feats, img_metas):
        ...
        if self.use_mss:
            fore_pred = []
            for i, pred_net in enumerate(self.ForePredNet):
                fore_pred.append(pred_net(mlvl_pts_feats[i]))
            enhance_pts_feats = self.mss(mlvl_pts_feats, fore_pred)
            mlvl_pts_feats = list(mlvl_pts_feats)
            for i in range(len(mlvl_pts_feats)):
                mlvl_pts_feats[i] = enhance_pts_feats[i] + mlvl_pts_feats[i]
    ...
    def loss(self,
        ...
        losses_cls, losses_bbox = multi_apply(self.loss_single, all_cls_scores, all_bbox_preds, all_gt_bboxes_list, all_gt_labels_list, img_metas_list, all_gt_bboxes_ignore_list)
        loss_dict = dict()
    ...
    def loss_single(self,
        ...
        code_weights = torch.tensor(self.code_weights, requires_grad=False, device=bbox_weights.device)
        bbox_weights = bbox_weights * code_weights
        loss_bbox = self.loss_bbox(bbox_preds[isnotnan, :self.code_size], normalized_bbox_targets[isnotnan, :self.code_size], bbox_weights[isnotnan, :self.code_size], avg_factor=num_total_pos)
    ...
    def _get_bboxes_single(self, cls_scores, bbox_preds):
        ...
        if self.test_cfg['post_center_range'] is not None:
            self.test_cfg['post_center_range'] = torch.tensor(self.test_cfg['post_center_range'], device=scores.device)
            mask = (final_box_preds[..., :3] >= self.test_cfg['post_center_range'][:3]).all(1)
            mask &= (final_box_preds[..., :3] <= self.test_cfg['post_center_range'][3:]).all(1)
            if self.test_cfg['score_threshold'] > 0:
                mask &= thresh_mask
