...
try:
    ...
    warnings.warn('`MultiScaleDeformableAttention` in MMCV has been moved to `mmcv.ops.multi_scale_deform_attn`, please update your MMCV')
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.use_dab:
                if self.no_sine_embed:
                    ...
                    query_sine_embed = gen_sineembed_for_position(reference_points_input)
...
class FUTR3DTransformer(BaseModule):
    ...
    def forward(self,
        ...
        if mlvl_pts_feats is not None:
            ...
            lvl_pos_embed_flatten = []
            spatial_shapes = []
            for lvl, (feat, mask, pos_embed) in enumerate(zip(mlvl_pts_feats, mlvl_masks, mlvl_pos_embeds)):
                bs, c, h, w = feat.shape
                spatial_shape = (h, w)
                spatial_shapes.append(spatial_shape)
                feat = feat.flatten(2).transpose(1, 2)
        ...
        pts_feat_flatten, mask_flatten, lvl_pos_embed_flatten, spatial_shapes, level_start_index, valid_ratios = self.feats_prepare(mlvl_pts_feats, mlvl_masks, mlvl_pos_embeds)
        rad_feat_flatten, rad_mask_flatten, rad_lvl_pos_embed_flatten, rad_spatial_shapes, rad_level_start_index, rad_valid_ratios = self.feats_prepare(radar_feats, mlvl_rad_masks, mlvl_rad_pos_embeds)
