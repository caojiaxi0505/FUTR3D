...
def create_groundtruth_database(dataset_class_name, data_path, info_prefix, info_path=None, mask_anno_path=None, used_classes=None, database_save_path=None, db_info_save_path=None, relative_path=True, add_rgb=False, lidar_only=False, bev_only=False, coors_range=None, with_mask=False):
    """根据原始数据生成ground truth数据库。

    参数:
        dataset_class_name (str): 输入数据集名称
        data_path (str): 数据路径
        info_prefix (str): info文件前缀
        info_path (str, 可选): info文件路径。默认: None
        mask_anno_path (str, 可选): mask标注路径。默认: None
        used_classes (list[str], 可选): 使用的类别。默认: None
        database_save_path (str, 可选): 数据库保存路径。默认: None
        db_info_save_path (str, 可选): db_info保存路径。默认: None
        relative_path (bool, 可选): 是否使用相对路径。默认: True
        with_mask (bool, 可选): 是否使用mask。默认: False
    """
...
class GTDatabaseCreater:
    """根据原始数据生成ground truth数据库的并行版本。串行版本请参考create_groundtruth_database。
    ...
    参数:
        ...
        relative_path (bool, 可选): 是否使用相对路径。默认: True
        with_mask (bool, 可选): 是否使用mask。默认: False
        num_worker (int, 可选): 并行worker数量。默认: 8
    """
