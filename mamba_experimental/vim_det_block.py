...
__all__ = ["VisionMambaDet", "get_vim_lr_decay_rate"]


class VisionMambaDet(VisionMamba, nn.Module):
    def __init__(
        self,
        img_size=224,
        patch_size=16,
    ...
    def forward_features(self, x, inference_params=None):
        ...
        if not self.if_bidirectional:
            ...
            for i in range(len(self.layers) // 2):
                ...
                hidden_states_b, residual_b = self.layers[i * 2 + 1](hidden_states.flip([1]), None if residual == None else residual.flip([1]), inference_params=inference_params)
        ...
        if self.last_layer_process == 'none':
            ...
            residual = fused_add_norm_fn(hidden_states, self.norm_f.weight, self.norm_f.bias, eps=self.norm_f.eps, residual=residual, prenorm=False, residual_in_fp32=self.residual_in_fp32)
